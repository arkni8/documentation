...
4. Open the controller `<collection-name>.js` file for the corresponding API request.
5. Add the following code, and make sure you replace the `<collection-name>` with the proper name of your collection:
  ...
  async find(ctx) {
    ...
    await Promise.all(
      data.map(async (item, index) => {
        const oneItem = await query.findOne({
          where: {
            id: item.id,
          },
        ...
        data[index].attributes.createdBy = {
          id: oneItem.createdBy.id,
          firstname: oneItem.createdBy.firstname,
          lastname: oneItem.createdBy.lastname,
        };
...
[Follow this issue](https://github.com/strapi/strapi/issues/12228) to understand more about this behavior.
